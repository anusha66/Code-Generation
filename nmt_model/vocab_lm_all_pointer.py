...
class Vocab(object):
    def __init__(self, src_sents, tgt_sents, vocab_size, freq_cutoff):
        ...
        #tgt_sents.extend(src_sents)
 
        src_sents.extend(tgt_sents)
        print('initialize comm vocabulary ..')
        self.comm = VocabEntry.from_corpus(src_sents, vocab_size, freq_cutoff)
       
        print('initialize all vocabulary ..')        
        self.all = VocabEntry()
...
#     pdb.set_trace()
    ...
    print('generated vocabulary, comm %d words, all %d words, source %d words, target %d words' % (len(vocab.comm), len(vocab.all), len(vocab.src), len(vocab.tgt)))
