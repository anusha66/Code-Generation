...
class Vocab(object):
    def __init__(self, src_sents, tgt_sents, vocab_size, freq_cutoff):
        ...
        total_words = set(self.src.word2id.keys()).union(self.tgt.word2id.keys())
    ...
    def __repr__(self):
        return 'Vocab(source %d words, target %d words, all %d words)' % (len(self.src), len(self.tgt), len(self.all))
...
#     tgt_sents, tgt_f_ids = read_corpus(args['--train-tgt'], source='tgt')
   ...
   # vocab = Vocab.build(src_sents, tgt_sents, int(args['--size']), int(args['--freq-cutoff']))
    ...
    src_sents, src_f_ids = read_corpus('/home/anushap/Code-Generation/nmt_model/data/code2code/nl_train.txt', source='src')
    tgt_sents, tgt_f_ids  = read_corpus('/home/anushap/Code-Generation/nmt_model/data/code2code/code_train.txt', source='tgt')
...
#     vocab = Vocab(src_sents, tgt_sents, int(args['--size']), int(args['--freq-cutoff'])) 
    vocab = Vocab(src_sents, tgt_sents, 10000, 2)    
  
    print('generated vocabulary, source %d words, target %d words, all %d words' % (len(vocab.src), len(vocab.tgt), len(vocab.all)))
    
    pickle.dump(vocab, open('data/vocab_all.bin', 'wb'))
